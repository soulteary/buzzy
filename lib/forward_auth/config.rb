...
module ForwardAuth
  class Config
    ...
    def trusted?(request)
      ...
      source_ips = extract_source_ips(request)
      return false if source_ips.empty?

      ip_trusted = trusted_ips.empty? || source_ips.any? { |source_ip| trusted_ips.any? { |net| net.include?(source_ip) } }
    ...
    def extract_source_ips(request)
      [ request.try(:remote_addr), request.try(:remote_ip) ]
