...
class Import
  FIX_LINK_HOSTS = {
    "fizzy.37signals.com" => "app.buzzy.do",
    "box-car.com" => "app.buzzy.do",
    "app.box-car.com" => "app.buzzy.do"
  }.freeze
        ...
        ActiveRecord::Base.no_touching do
          Current.with(account: account) do
            begin
              ...
              Reaction.skip_callback(:commit, :after, :notify_recipients_later)
          ...
          fragment.find_all("a[href]").each do |link|
            ...
            if uri
              ...
              if params[:controller] == "cards" && params[:action] == "show" && params[:id] && mapping[:cards][params[:id].to_i]
                c = mapping[:cards][params[:id].to_i]
                uri.path = Rails.application.routes.url_helpers.user_board_card_path(c.board.url_user, c.board, c)
              elsif params[:controller] == "boards" && params[:action] == "show" && params[:id] && mapping[:boards][params[:id].to_i]
                b = mapping[:boards][params[:id].to_i]
                uri.path = Rails.application.routes.url_helpers.user_board_path(b.url_user, b)
              end
