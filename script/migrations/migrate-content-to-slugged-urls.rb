...
domains = {
  "production" => "app.buzzy.do",
  "beta" => ENV.fetch("APP_FQDN", "beta1.buzzy-beta.com"),
  "staging" => "app.buzzy-staging.com"
}
...
ApplicationRecord.with_each_tenant do |tenant|
  ...
  Card.find_each do |card|
    puts "### Processing card #{card.id} in #{Rails.application.routes.url_helpers.user_board_card_path(card.board.url_user, card.board, card)}"
    fix_attachments(card.description)
    card.reload

    old_body = card.description.body.to_s
    if match = regex.match(old_body)
      puts "URL found in card #{card.id} in #{Rails.application.routes.url_helpers.user_board_card_path(card.board.url_user, card.board, card)}"
  ...
  Comment.find_each do |comment|
    puts "### Processing comment #{comment.id} in #{Rails.application.routes.url_helpers.user_board_card_path(comment.card.board.url_user, comment.card.board, comment.card)}"
    fix_attachments(comment.body)
    comment.reload

    old_body = comment.body.body.to_s
    if match = regex.match(old_body)
      puts "URL found in comment #{comment.id} in #{Rails.application.routes.url_helpers.user_board_card_path(comment.card.board.url_user, comment.card.board, comment.card)}"
      new_body = old_body.gsub(regex, "://#{domain}/#{account_id}/")
