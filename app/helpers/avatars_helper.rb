...
module AvatarsHelper
  ...
  def avatar_tag(user, hidden_for_screen_reader: false, **options)
    link_to user_path(user, script_name: nil), class: class_names("avatar btn btn--circle", options.delete(:class)), data: { turbo_frame: "_top" },
  ...
  def mail_avatar_tag(user, size: 48, **options)
    if user.avatar.attached?
      image_tag user_avatar_url(user, script_name: nil), alt: user.name, class: "avatar", size: size, **options
    else
  ...
  def avatar_image_tag(user, **options)
    image_tag user_avatar_path(user, script_name: nil), aria: { hidden: "true" }, size: 48, title: user.name, **options
  end
end
