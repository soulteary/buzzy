module My::MenuHelper
  ...
  def my_menu_board_item(board)
    my_menu_item("board", board) do
      label = "#{board.name} (@#{board.creator.name})"
      link_to(tag.span(label, class: "overflow-ellipsis"), polymorphic_path(board), class: "popup__btn btn")
    end
  end
  ...
  def my_menu_user_item(user)
    my_menu_item("person", user) do
      link_to(tag.span(user.name, class: "overflow-ellipsis"), user_path(user, script_name: nil), class: "popup__btn btn")
    end
  end
