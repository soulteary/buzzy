module ColumnsHelper
  ...
  def user_scoped_column_left_position_path(column)
    user_board_column_left_position_path(column.board.url_user, column.board, column)
  end

  def user_scoped_column_right_position_path(column)
    user_board_column_right_position_path(column.board.url_user, column.board, column)
  end

  def user_scoped_column_drop_path(board, type:, card_id:, column_id: nil)
    case type
    when :stream
      user_board_columns_card_drops_stream_path(board.url_user, board, card_id)
    when :not_now
      user_board_columns_card_drops_not_now_path(board.url_user, board, card_id)
    when :closure
      user_board_columns_card_drops_closure_path(board.url_user, board, card_id)
    when :column
      user_board_columns_card_drops_column_path(board.url_user, board, card_id, column_id: column_id)
    else
      raise ArgumentError, "Unsupported drop type: #{type}"
    end
  end
end
