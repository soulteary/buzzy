class Sessions::MenusController < ApplicationController
  ...
  skip_before_action :redirect_tenanted_request

  layout "public"

  def show
    # 仅当当前账号下已有可用用户时，才从 account 作用域跳回首页；
    # 若 Current.user 为空（常见于跨账号身份），必须允许进入 session menu 选择入口，避免 302 循环。
  ...
  private
    ...
    def create_account_for_forward_auth_identity
      cfg = forward_auth_config
      owner_name = request.headers["X-Auth-Name"].to_s.strip.presence || email_local_part(Current.identity.email_address)
