class ApplicationController < ActionController::Base
  ...
  helper_method :super_admin?, :account_single_user?, :viewing_other_user_as_limited_viewer?, :board_accessed_via_mention?, :card_editable_by_current_user?, :card_deletable_by_current_user?, :card_pinnable_by_current_user?, :card_from_other_account?, :pinning_user_for, :assignees_read_only_for?, :current_notification_user, :board_display_cards, :board_administerable_by_current_user?, :board_editable_by_current_user?
  ...
  def card_editable_by_current_user?(card)
    CardPolicy.editable?(Current.user, card, board_accessed_via_mention: board_accessed_via_mention?, super_admin: super_admin?)
  end

  # 仅卡片的创建者或账号管理员在非「仅因提及/公开」访问时可见并使用删除卡片功能；其他用户不展示删除入口
  def card_deletable_by_current_user?(card)
    CardPolicy.deletable?(Current.user, card, board_accessed_via_mention: board_accessed_via_mention?, super_admin: super_admin?)
  end
  ...
  def card_pinnable_by_current_user?(card)
    CardPolicy.pinnable?(
      ...
      board_accessed_via_mention: board_accessed_via_mention?,
      user_before_fallback: Current.user_before_fallback
    )
  end
  ...
  def card_from_other_account?(card = nil)
    CardPolicy.from_other_account?(Current.user, card || @card, super_admin: super_admin?)
  end

  # 看板已锁定可编辑性时，禁止普通用户修改看板及其中卡片/列（super_admin 可绕过）
  def ensure_board_editable
    board = @board || @card&.board || @column&.board
    head :forbidden if board.present? && !super_admin? && !board.editable_by?(Current.user)
  end
  ...
  # 看板管理权限：仅 super_admin 或可管理该看板的用户可通过；供 BoardScoped、BoardsController、WebhooksController 等共用
  def ensure_permission_to_admin_board
    return if super_admin?
    head :forbidden if Current.user.blank? || !Current.user.can_administer_board?(@board)
  end

  # 卡片分配人（assignees）是否只读：与 ensure_can_administer_card_assignments 等控制器规则一致，视图统一调用避免重复布尔表达式
  def assignees_read_only_for?(card)
    CardPolicy.assignees_read_only?(
      Current.user, card,
      board_accessed_via_mention: board_accessed_via_mention?,
      from_other_account: card_from_other_account?(card),
      viewing_other_user_as_limited_viewer: viewing_other_user_as_limited_viewer?,
      super_admin: super_admin?
    )
  end

  # 看板内用于展示的卡片集合：仅因提及/指派进入时仅返回该用户可见的已发布卡片，否则为看板全部卡片
  def board_display_cards
    return Card.none if @board.blank?
    if @board_accessed_via_mention && Current.user.present?
      Current.user.cards_visible_in_board_for_limited_view(@board).published
    else
      @board.cards
    end
  end

  def board_administerable_by_current_user?(board)
    super_admin? || (Current.user.present? && Current.user.can_administer_board?(board))
  end

  def board_editable_by_current_user?(board)
    super_admin? || board.editable_by?(Current.user)
  end

  # Returns a user record that can receive notification streams in the UI.
  # When visiting a foreign account (Current.user is nil), fall back to any
  # active membership under the same identity so the tray can still render.
  def current_notification_user
    return @current_notification_user if defined?(@current_notification_user)

    @current_notification_user =
      if Current.user.present?
        Current.user
      elsif Current.identity.present?
        Current.identity.users.active.order(:id).first
      end
  end
