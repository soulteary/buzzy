class UsersController < ApplicationController
  ...
  helper_method :name_locked_by_forward_auth?
  ...
  def update
    if deny_name_change_when_forward_auth_name_present?
      respond_to do |format|
  ...
  private
    ...
    def deny_name_change_when_forward_auth_name_present?
      return false unless name_locked_by_forward_auth?

      submitted_name = params.dig(:user, :name)
      return false if submitted_name.nil?
      return false if submitted_name.to_s == @user.name.to_s

      @user.assign_attributes(user_params.except(:name))
      @user.errors.add(:name, t("users.edit.name_locked_by_forward_auth"))
      true
    end

    def name_locked_by_forward_auth?
      forward_auth_name_present_and_trusted?
    end

    def forward_auth_name_present_and_trusted?
      cfg = Rails.application.config.forward_auth
      return false unless cfg.is_a?(ForwardAuth::Config) && cfg.enabled?
      return false unless cfg.trusted?(request)

      request.headers["X-Auth-Name"].to_s.strip.present?
    end
