class Cards::CommentsController < ApplicationController
  ...
  # 跨账号评论：当前身份在看板所属账号下可能无 User，set_board 回退会先找到看板，再在此补全 Current.user，避免 require_user_in_account / ensure_can_access_account 直接返回空 frame
  allow_unauthorized_access only: :create
  skip_before_action :require_user_in_account, only: :create
  before_action :set_user_for_cross_account_comment, only: :create
