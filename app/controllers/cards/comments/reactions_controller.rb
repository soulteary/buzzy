class Cards::Comments::ReactionsController < ApplicationController
  ...
  # 跨账号时与评论一致：先通过 set_board 回退找到看板并补全 Current.user，再渲染 new/create，避免返回空 frame
  allow_unauthorized_access only: %i[ index new create ]
  skip_before_action :require_user_in_account, only: %i[ index new create ]
  before_action :set_user_for_cross_account_comment, only: %i[ index new create ]
