module Board::Accessible
  ...
  def accessible_to?(user)
    return true if user.present? && user.account_id == account_id && user.account.users.active.where.not(role: :system).one?
  ...
  private
    ...
    def notifications_for_user(user)
      ...
      # 3. Notification->Reaction->Card (reactable_type = 'Card')
      # 4. Notification->Reaction->Comment->Card (reactable_type = 'Comment')
      #
      # Notification->Event->Mention->Card and Notification->Event->Mention->Comment->Card are
      # handled by destroying mentions_for_user.
      uuid_adapter = (connection.adapter_name == "SQLite" ? :sqlite3 : :trilogy)
      uuid_type = ActiveRecord::Type.lookup(:uuid, adapter: uuid_adapter)
      ...
      user.notifications
        ...
        .joins("LEFT JOIN reactions ON notifications.source_id = reactions.id AND notifications.source_type = 'Reaction'")
        .joins("LEFT JOIN cards AS reaction_cards ON reactions.reactable_id = reaction_cards.id AND reactions.reactable_type = 'Card'")
        .joins("LEFT JOIN comments AS reaction_comments ON reactions.reactable_id = reaction_comments.id AND reactions.reactable_type = 'Comment'")
        .joins("LEFT JOIN cards AS reaction_comment_cards ON reaction_comments.card_id = reaction_comment_cards.id")
        .where(
          "(notifications.source_type = 'Event' AND events.eventable_type = 'Card' AND event_cards.board_id = ?) OR
           (notifications.source_type = 'Event' AND events.eventable_type = 'Comment' AND event_comment_cards.board_id = ?) OR
           (notifications.source_type = 'Reaction' AND reactions.reactable_type = 'Card' AND reaction_cards.board_id = ?) OR
           (notifications.source_type = 'Reaction' AND reactions.reactable_type = 'Comment' AND reaction_comment_cards.board_id = ?)",
          board_id_binary, board_id_binary, board_id_binary, board_id_binary
        )
    end
