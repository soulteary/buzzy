class Filter < ApplicationRecord
  ...
  def cards
    @cards ||= begin
      # 筛选「分配给我的任务」时，除可访问看板的卡片外，也包含我因「被指派 / 被提及」而可访问的卡片（含跨账号）。
      # 使用子查询合并范围，避免不同 join 结构直接 or 导致结果不完整。
      result = if assignees.present? && assignees.ids.include?(creator.id)
        Card.where(id: creator.accessible_cards.select(:id))
          .or(Card.where(id: creator.cards_accessible_via_assignment.select(:id)))
          .or(Card.where(id: creator.cards_accessible_via_mention.select(:id)))
      else
