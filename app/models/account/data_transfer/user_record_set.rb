class Account::DataTransfer::UserRecordSet < Account::DataTransfer::RecordSet
  ...
  private
    ...
    def import_batch(files)
      ...
      # Single user per account: keep at most one non-system user (first), plus all system users
      non_system = batch_data.reject { |h| h["role"].to_s == "system" }
      if non_system.size > 1
        first_real_id = non_system.first["id"]
        batch_data = batch_data.select { |h| h["role"].to_s == "system" || h["id"] == first_real_id }
      end
