...
ActiveSupport.on_load(:active_storage_attachment) do
  ...
  private
    ...
    def whitelisted_for_cross_account?
      # Template account blobs can be reused cross-tenant.
      return true if Storage::TEMPLATE_ACCOUNT_ID.present? && blob&.account_id == Storage::TEMPLATE_ACCOUNT_ID

      # Comment body embeds: allow blob from the comment creator's account or any account
      # under the same identity (cross-account comment with image uploaded in commenter's other workspace).
      if record.is_a?(ActionText::RichText) && record.record_type == "Comment" && record.record_id.present? && blob.present?
        comment = record.record
        creator = comment.creator
        if creator.present?
          return true if blob.account_id == creator.account_id
          identity = creator.identity
          return true if identity.present? && identity.users.where(account_id: blob.account_id).exists?
        end
      end

      # Card image: allow blob from card creator's account or same identity (e.g. card moved
      # across accounts, or variant job runs in board account while blob was created in creator's other account).
      if record_type == "Card" && name == "image" && record.present? && blob.present?
