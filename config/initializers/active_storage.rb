...
# using `preprocessed: true` variants.
...
# Compatibility shim:
# historical variant URLs may include { process: :immediately }, which was valid
# in older app code but is not a real image operation for Vips.
# Strip this key while decoding variation payloads to avoid 500s on stale links.
module ActiveStorageVariationLegacyProcessCompatibility
  def initialize(transformations)
    super(transformations.to_h.except("process", :process))
  end
end

Rails.application.config.to_prepare do
  ActiveStorage::Variation.prepend(ActiveStorageVariationLegacyProcessCompatibility) unless ActiveStorage::Variation < ActiveStorageVariationLegacyProcessCompatibility
end
