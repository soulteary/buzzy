Rails.application.routes.draw do
  ...
  resources :users, path: "users" do
    ...
    resources :boards, only: [ :show, :edit, :update ], controller: "boards", param: :id do
      ...
      resources :cards, only: [ :show, :edit, :update, :destroy ], controller: "cards", param: :id do
  ...
  resources :boards do
    scope module: :boards do
      ...
      # 约束 :id 为 UUID，避免 /columns/not_now 与 /columns/closed 被当作 column :id 匹配导致 404
      resources :columns, constraints: { id: /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i }
    end
  ...
  # 旧路径 /user/:id 或 /:account_slug/user/:id 统一重定向到 /users/:id
  get "/user/:id", to: redirect("/users/%{id}"), constraints: { id: /[0-9a-f-]+/i }
