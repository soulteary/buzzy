...
Rails.application.configure do
  ...
  # Bullet: N+1 / 未使用预加载检测（见 docs/mysql-performance-checklist.md）
  config.after_initialize do
    if ENV["BULLET_ENABLED"] != "false"
      require "bullet"
      Bullet.enable = true
  ...
  # Email: prefer SMTP from ENV (e.g. .env), then letter_opener, else don't send
  ...
  config.hosts = [
    ...
    "localhost",
    "127.0.0.1",
    /buzzy-\d+/,   # review apps: buzzy-123, buzzy-456:3000
    /.*\.ts\.net/, # tailscale serve: hostname.tail1234.ts.net
    /.*\.nip\.io/  # nip.io for mobile apps
  ]

  # Base URL for links in emails (when set in .env); otherwise use default host
