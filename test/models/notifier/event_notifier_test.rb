...
class Notifier::EventNotifierTest < ActiveSupport::TestCase
  ...
  test "creates comment notifications for watchers and assignees other than the event creator" do
    notifications = Notifier.for(events(:layout_commented)).notify

    assert_equal [ users(:jz), users(:kevin) ].sort_by(&:id), notifications.map(&:user).sort_by(&:id)
  end

  test "creates comment notifications for assignees even when not watching" do
    cards(:layout).unwatch_by(users(:jz))
    notifications = Notifier.for(events(:layout_commented)).notify

    assert_includes notifications.map(&:user), users(:jz)
  end
  ...
  test "comment events also notify same identity users in other accounts" do
    other_account_user = User.create!(
      name: "Kevin Other Account",
      role: :member,
      identity: users(:kevin).identity,
    ...
    assert_equal [ users(:jz), users(:kevin), other_account_user ].sort_by(&:id), notifications.map(&:user).sort_by(&:id)
    assert notifications.all? { |notification| notification.creator == users(:david) }
  end
