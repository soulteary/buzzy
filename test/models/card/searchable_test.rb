...
class Card::SearchableTest < ActiveSupport::TestCase
  ...
  test "card search falls back to like for cjk terms" do
    card = @board.cards.create!(title: "中文卡片搜索", status: "published", creator: @user)
    results = Card.mentioning("卡片", user: @user)
    assert_includes results, card
  end

  test "card search uses searchable boards when direct board_ids are empty" do
    card = @board.cards.create!(title: "search by fallback board scope", status: "published", creator: @user)
    # 单用户账号下可能没有 accesses 记录；模拟 board_ids 为空但仍应可在本账号看板内搜索。
    @user.accesses.delete_all

    results = Card.mentioning("fallback", user: @user)
    assert_includes results, card
  end
