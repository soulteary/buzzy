...
class SignupTest < ActiveSupport::TestCase
  ...
  test "#complete does not create second account when identity already has an owner" do
    Current.without_account do
      # jason 在 fixtures 中已是 37s 的 owner
      identity = identities(:jason)
      existing_owner = User.find_by(identity: identity, role: :owner)
      assert existing_owner, "jason should be owner of an account in fixtures"
      account_count_before = Account.count
      user_count_before = User.where(identity: identity).count

      signup = Signup.new(full_name: "Jason Again", identity: identity)
      Signup.any_instance.expects(:create_tenant).never
      Signup.any_instance.expects(:create_account).never

      assert signup.complete
      assert_equal existing_owner.account, signup.account
      assert_equal existing_owner, signup.user
      assert_equal account_count_before, Account.count, "should not create a second account"
      assert_equal user_count_before, User.where(identity: identity).count, "should not create a second user"
    end
  end
end
