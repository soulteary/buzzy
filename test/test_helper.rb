...
module ActiveSupport
  class TestCase
    parallelize_options = { workers: :number_of_processors }
    if ActiveSupport::TestCase.method(:parallelize).parameters.any? { |(_type, name)| name == :work_stealing }
      parallelize_options[:work_stealing] = ENV["WORK_STEALING"] != "false"
    end
...
class ActionDispatch::IntegrationTest
  setup do
    integration_session.default_url_options[:script_name] = accounts("37s").slug
  end
...
class ActionDispatch::SystemTestCase
  setup do
    self.default_url_options[:script_name] = accounts("37s").slug
  end
end
