...
class Cards::CommentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "cross-account viewer can comment on public card with foreign user id in path" do
    ...
    assert_difference -> { card.comments.count }, +1 do
      post user_board_card_comments_path(foreign_user, board, card), params: { comment: { body: "Cross-account via foreign path user." } }, as: :turbo_stream
    end
    assert_response :success
  end
  ...
  test "create stores markdown source in body_markdown" do
    card = cards(:logo)
    markdown = "111233\n\n- item 1"

    assert_difference -> { card.comments.count }, +1 do
      post user_board_card_comments_path(card.board.url_user, card.board, card), params: { comment: { body: markdown } }, as: :json
    end

    assert_response :created
    assert_equal markdown, Comment.last.rich_text_body.body_markdown
    assert_includes Comment.last.body.to_plain_text, "111233"
    assert_includes Comment.last.body.to_plain_text, "item 1"
  end
