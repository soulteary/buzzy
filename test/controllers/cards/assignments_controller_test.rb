...
class Cards::AssignmentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "new" do
    card = cards(:logo)
    get new_board_card_assignment_path(card.board, card)
  ...
  test "create" do
    ...
    assert_changes "cards(:logo).reload.assigned_to?(users(:david))", from: false, to: true do
      post board_card_assignments_path(card.board, card), params: { assignee_id: users(:david).id }, as: :turbo_stream
  ...
  test "create as JSON" do
    ...
    post board_card_assignments_path(card.board, card), params: { assignee_id: users(:david).id }, as: :json
