...
class Cards::PinsControllerTest < ActionDispatch::IntegrationTest
    ...
    assert_changes -> { cards(:layout).pinned_by?(users(:kevin)) }, from: false, to: true do
      perform_enqueued_jobs do
        assert_turbo_stream_broadcasts([ users(:kevin), :pins_tray ], count: 1) do
          post board_card_pin_path(cards(:layout).board, cards(:layout)), as: :turbo_stream
        end
      end
    end
  ...
  test "create as JSON" do
    ...
    post board_card_pin_path(card.board, card), as: :json
  ...
  test "destroy" do
    assert_changes -> { cards(:shipping).pinned_by?(users(:kevin)) }, from: true, to: false do
      perform_enqueued_jobs do
        assert_turbo_stream_broadcasts([ users(:kevin), :pins_tray ], count: 1) do
          delete board_card_pin_path(cards(:shipping).board, cards(:shipping)), as: :turbo_stream
        end
      end
    end
  ...
  test "destroy as JSON" do
    ...
    delete board_card_pin_path(card.board, card), as: :json
