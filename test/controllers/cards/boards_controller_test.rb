...
class Cards::BoardsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "update changes card board" do
    ...
    assert_changes -> { card.reload.board }, from: card.board, to: new_board do
      put user_board_card_board_path(card.board.url_user, card.board, card), params: { target_board_id: new_board.id }
    end
  ...
  test "update as JSON" do
    ...
    put user_board_card_board_path(card.board.url_user, card.board, card), params: { target_board_id: new_board.id }, as: :json
