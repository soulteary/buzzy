...
class Cards::TaggingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "new" do
    get new_board_card_tagging_path(boards(:writebook), cards(:logo))
    assert_response :success
  end

  test "toggle tag on" do
    assert_changes "cards(:logo).tagged_with?(tags(:mobile))", from: false, to: true do
      post board_card_taggings_path(boards(:writebook), cards(:logo)), params: { tag_title: tags(:mobile).title }, as: :turbo_stream
  ...
  test "toggle tag off" do
    assert_changes "cards(:logo).tagged_with?(tags(:web))", from: true, to: false do
      post board_card_taggings_path(boards(:writebook), cards(:logo)), params: { tag_title: tags(:web).title }, as: :turbo_stream
      assert_turbo_stream action: :replace, target: dom_id(cards(:logo), :tags)
    end
  end
  ...
  test "toggle tag on as JSON" do
    ...
    post board_card_taggings_path(card.board, card), params: { tag_title: tags(:mobile).title }, as: :json
  ...
  test "toggle tag off as JSON" do
    ...
    post board_card_taggings_path(card.board, card), params: { tag_title: tags(:web).title }, as: :json
