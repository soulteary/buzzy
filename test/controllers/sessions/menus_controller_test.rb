...
class Sessions::MenusControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show with exactly one account" do
    ...
    Current.without_account do
      ...
      user = @identity.users.create!(account: account, name: "Kevin")
    end
    ...
    assert_redirected_to user_url(user, script_name: nil)
  end
  ...
  test "show under account scope without user in current account does not redirect back to session menu" do
    sign_in_as @identity
    @identity.users.delete_all
    account = Account.create!(name: "Cross Account")
    user = @identity.users.create!(account: account, name: "Kevin")
