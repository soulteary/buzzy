...
class UsersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "update oneself" do
    ...
    assert_redirected_to profile_user_path(users(:kevin))
  ...
  test "update other as admin" do
    ...
    assert_redirected_to profile_user_path(users(:david))
  ...
  test "update with bio over 140 characters shows validation error" do
    sign_in_as :kevin

    put user_path(users(:kevin)), params: { user: { bio: "a" * 141 } }
  ...
  test "index redirects to current user" do
    sign_in_as :kevin

    get users_path
    assert_redirected_to user_path(users(:kevin), script_name: nil)
  end
  ...
  test "destroy single user (self) is not allowed" do
    sign_in_as :mike
    integration_session.default_url_options[:script_name] = accounts("initech").slug

    # Initech has only mike (and system); cannot remove self
    assert_no_difference -> { User.active.count } do
      delete user_path(users(:mike))
    end
    assert_redirected_to account_settings_path
    assert flash[:alert].present?, "should set alert when destroying the only user"
  end
  ...
  test "cannot update display name when trusted forward auth request includes X-Auth-Name" do
    sign_in_as :kevin
    original_config = Rails.application.config.forward_auth
    ...
    put user_path(users(:kevin)), params: { user: { name: "Gateway Override" } }, headers: { "X-Auth-Name" => "Gateway Name" }

    assert_response :unprocessable_entity
    assert_equal "Kevin", users(:kevin).reload.name
    assert_in_body I18n.t("users.edit.name_locked_by_forward_auth")
  ensure
    Rails.application.config.forward_auth = original_config || ForwardAuth::Config.new(enabled: false)
  end
