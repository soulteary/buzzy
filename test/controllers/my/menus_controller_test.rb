...
class My::MenusControllerTest < ActionDispatch::IntegrationTest
  ...
  test "etag invalidates when followed users change" do
    get my_menu_path
    assert_response :success
    etag = response.headers["ETag"]

    # 菜单「用户」现在展示的是关注的人，关注列表变化时 etag 应失效
    UserFollow.create!(follower: @user, followee: users(:david))
  ...
  test "turbo frame request always returns 200 with body so frame is not replaced with empty content" do
    get my_menu_path
    assert_response :success
    etag = response.headers["ETag"]

    get my_menu_path, headers: { "If-None-Match" => etag, "Turbo-Frame" => "my_menu" }
    assert_response :success
    assert_select "turbo-frame#my_menu", count: 1
  end
  ...
  test "cross-account board link uses board account script_name" do
    cross_board = boards(:miltons_wish_list)
    UserFollow.find_or_create_by!(follower: @user, followee: users(:mike))

    get my_menu_path
    assert_response :success

    expected_href = user_board_path(cross_board.url_user, cross_board, script_name: cross_board.account.slug)
