...
class Boards::EntropiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "update" do
    assert_no_difference -> { Current.account.entropy.reload.auto_postpone_period } do
      put user_board_entropy_path(@board.url_user, @board, format: :turbo_stream), params: { board: { auto_postpone_period: 123.days } }
  ...
  test "update requires board admin permission" do
    ...
    put user_board_entropy_path(@board.url_user, @board, format: :turbo_stream), params: { board: { auto_postpone_period: 1.day } }
