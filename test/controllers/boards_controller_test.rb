...
class BoardsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show falls back when route user_id belongs to another account" do
    board = boards(:writebook)
    foreign_user = users(:mike)

    get "/#{accounts("37s").slug}/users/#{foreign_user.id}/boards/#{board.id}"

    assert_response :success
  end

  test "mentioned user can open non-public board show and sees only mentioned cards" do
    ...
    assert users(:david).cards_visible_in_board_for_limited_view(board).exists?(id: card.id), "david is mentioned on secret_card"

    get user_board_path(board.url_user, board)
    assert_response :success
    assert_match card.title, response.body
    assert_equal 1, users(:david).cards_visible_in_board_for_limited_view(board).count, "limited view shows only mention/assignment cards"
  end

  test "mentioned user cannot open non-public board when they have no cards in it" do
    logout_and_sign_in_as :jz
    board = boards(:private)
    assert_not board.accessible_to?(users(:jz)), "jz should not have board access"
    assert_not users(:jz).cards_visible_in_board_for_limited_view(board).exists?, "jz has no mention/assignment on this board"
  ...
  test "super admin can update board across account without account user" do
    ...
    Buzzy.stub(:admin_emails, [ email ].to_set) do
      patch user_board_path(board.url_user, board), params: {
        board: { name: "Super admin renamed board" }
      }
    end

    assert_redirected_to edit_user_board_path(board.url_user, board)
    assert_equal "Super admin renamed board", board.reload.name
  end

  test "super admin can open board edit across account without account user" do
    logout_and_sign_in_as :mike
    board = boards(:writebook)
    email = users(:mike).identity.email_address
    ...
    assert_response :success
    assert_select "button#log_in:not([disabled])"
  end
end
