...
module ForwardAuth
  class ConfigTest < ActiveSupport::TestCase
    Request = Struct.new(:remote_ip, :remote_addr, :headers, keyword_init: true)
    ...
    test "trusted? trusts proxy source IP when remote_ip is client IP" do
      config = Config.new(enabled: true, trusted_ips: [ "172.16.0.0/12" ])
      request = Request.new(
        remote_ip: "124.64.22.222",
